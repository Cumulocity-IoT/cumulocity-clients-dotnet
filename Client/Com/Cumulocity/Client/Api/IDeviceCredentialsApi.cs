...
public interface IDeviceCredentialsApi
{
	...
	/// ⓘ Info: A bulk registration creates an elementary representation of the device. Then, the device needs to update it to a full representation with its own status. The device is ready to use only after it is updated to the full representation. Also see <see href="https://www.cumulocity.com/docs/device-management-application/registering-devices/#security-token-policy" langword="Device management > Device management application > Registering devices > Single device registration > Security token policy" /> and <see href="https://www.cumulocity.com/docs/device-integration/rest/#integration-life-cycle" langword="Device management > Device integration > Device integration using REST > Integration life cycle" />. <br />
	/// ⓘ Info: A bulk registration for <see href="#tag/Device-enrollment" langword="certificate provisioning" /> creates temporary credentials for devices to authenticate <br />
