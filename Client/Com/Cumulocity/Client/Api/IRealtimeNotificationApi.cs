...
/// ⚠️ Important: Real-time notification is based on the Bayeux protocol where idle clients will have their sessions terminated after a set interval, defaulting to 2 hours.An idle client is one that has not received any messages from the server, nor polled the <c>/meta/connect</c> endpoint.To avoid session termination clients should poll <c>/meta/connect</c> and follow the instructions for <see href="#tag/Real-time-notification-API/" langword="Long-running connections" />.Additional information regarding the Bayeux protocol can be found in the CometD documentation under <see href="https://docs.cometd.org/current3/reference/#_bayeux" langword="Appendix C: The Bayeux Protocol Specification 1.0" />. <br />
/// <br /> Handshake <br />
